...
#SBATCH --job-name=CNS-lr0.0002-test      # 任务名称
#SBATCH --partition=gpu-a100-8     # 请求的分区
#SBATCH --nodes=1                         # 请求一个节点
#SBATCH --ntasks=1                        # 请求一个任务
#SBATCH --cpus-per-task=1                # 每个任务使用一个 CPU
#SBATCH --gres=gpu:1
...
# ./train.sh configs/monodgp.yaml
./test.sh configs/monodgp.yaml
